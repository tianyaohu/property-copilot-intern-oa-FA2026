...
2. **Performance at density** — how you keep the map smooth with all markers visible (clustering, viewport rendering, etc.) and what you observed.
   - *Draft:* Considered clustering and declined it at n=50: fifty cached `divIcon` markers are trivial for the DOM (pan/zoom stays smooth with all markers visible), and cluster bubbles would hide the "every marker visible" behaviour the product wants. Threshold: at ~300+ overlapping markers I'd add react-leaflet-cluster or canvas rendering; viewport-driven fetching already bounds what renders.
   ...
   - *Draft:* Guards sized from measurement: the largest UI-reachable viewport estimates to ~726 partitions, so the server rejects boxes above 800 — using an arithmetic estimate that runs *before* cell enumeration, since a world-scale box would otherwise allocate ~33M strings. Per-partition pagination and a 200-item `truncated` cap protect correctness and payload size at real data volumes.
